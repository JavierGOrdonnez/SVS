...
| `src/migration/build_dashboard_data.py` (formerly `build_migration_dashboard_data.py`) | `migration_spain.csv`, `population_spain_midyear_5yr.csv` | `build()` consumed by `src/analysis/build_dashboard.py` → `docs/data/migration.json` (incl. T76's `stock_age_pyramid_dz_ma`/`dz_ma_working_age_trend` keys); legacy stdout-JS `main()` kept for standalone use | T-mig-tab,T76 | x |
