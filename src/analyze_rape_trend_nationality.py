...
""")
  ...
  ⚠ LIMITACIÓN: No existe ninguna fuente española que mida directamente
  R_desconocido. El FRA 2014 (fuente §6) indica que España tiene tasas de
  denuncia "por debajo de la media europea" — pero esa cifra (25-30%) se
  refiere a la media europea general de violencia sexual con pareja, NO
  a una tasa para violaciones por desconocidos en España. Aplicarla como
  R_desconocido sería una extrapolación sin soporte directo.

  Lo que sí podemos derivar con las fuentes disponibles:

  Si la mezcla (9.9%) se descompone y queremos acotar R_conocido, necesitamos
  asumir un valor para R_desconocido. Los escenarios son:

    Escenario A (R_desconocido = 20%):
      0.812 × R_conocido + 0.188 × 0.20 = 0.099
      R_conocido ≈ (0.099 - 0.038) / 0.812 ≈ 7.5%

    Escenario B (R_desconocido = 30%):
      0.812 × R_conocido + 0.188 × 0.30 = 0.099
      R_conocido ≈ (0.099 - 0.056) / 0.812 ≈ 5.3%

    Escenario C (R_desconocido = R_conocido, sin diferencia):
      La tasa global = 9.9% para todas las relaciones

  RESULTADO (estimación con incertidumbre alta):
    • Violación por desconocido: probablemente > 9.9% (más alta que la media),
      rango plausible 15-30% — pero sin dato directo español
    • Violación por conocido no-pareja: probablemente < 9.9%, rango 5-8%
    • Violación por pareja: 11.3% (medido directamente, GEAV/MIR 2020)

  NOTA: La lógica de que "agresor desconocido → mayor denuncia" es consistente
  con la literatura internacional (UNODC, Campbell et al.) pero NO está
  directamente medida para España. Solo podemos decir que la dirección del
  efecto es probable, no su magnitud.

  IMPLICACIÓN PARA EL ANÁLISIS DE NACIONALIDAD:
  Si los extranjeros tienden a cometer más agresiones como desconocidos
  (patrón consistente con datos de agresiones grupales: 60% desconocidos),
  y si las agresiones por desconocidos tienen mayor tasa de denuncia, entonces:

    → Los datos policiales/judiciales podrían SOBREREPRESENTAR a perpetradores
      extranjeros en relación con su peso real en las agresiones totales.
    → El 81.2% por conocidos (mayoritariamente en contexto íntimo) tiene
      tasa de denuncia baja → gran parte queda invisible en estadísticas.

  Esta hipótesis es plausible y consistente con los datos, pero no puede
  cuantificarse con las fuentes disponibles en este repositorio.
  Requeriría microdatos de Macroencuesta enlazados con datos policiales.
""")
