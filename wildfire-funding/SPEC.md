...
| T9 | ~ | data | Executed vs. budgeted (C9) — schema extended (`wff_spending.csv` now has `spend_type`/`program_code`/`program_name`; existing rows backfilled `presupuestado`). First real presupuestado/liquidado pairs landed: Galicia 2023 (4 partial capital-investment lines, presupuestado only), País Vasco/Bizkaia 2023 (a real €0-presupuestado-vs-€1.34M-liquidado gap on a wildfire-specific project), Cataluña 2020-2021 (program 223, broader-than-wildfire scope). All `confidence=low` pending `source_ref` re-verification — see SOURCES_INDEX.md. Sequential foreground pass continuing per user direction: Extremadura → Madrid → Aragón → Castilla-La Mancha → Castilla y León → Galicia (deepen). No more background agents (recursive-sub-agent + crash-survivability problems both surfaced this pass). |
| T10 | ~ | infra/data | Tier-1 official all-CCAA total-budget parser (`parsers/parse_hacienda_totals.py`) — Hacienda's `PublicacionPresupuestos` (2013-2026 initial) and `PublicacionLiquidaciones` (executed through 2024) portals both expose bulk Excel downloads, not just interactive query forms. Not wildfire-specific (CCAA-total level), but authoritative and multi-year; will eventually replace `wff_denominators.csv`'s press-sourced `total_budget_eur`. In progress. |
