...
| 2020 | 13,174 | 1,596 | **exact match** vs Informe 2020 (total 13,174, `agresion_sexual_con_penetracion` 1,596) |
...
**Update (T83/T87, 2026-07-29): 2017/2018 Informe PDFs now contribute nationality data, headline total still N/A.** The "2017/2018 Informe PDFs failed to parse cleanly on their older layout" gap noted above is only true for the headline "hechos conocidos" typology table (a different, older multi-year layout than 2019+ uses — 2017 returns nothing, 2018 returns an internally-inconsistent 81-vs-703 mismatch). The SAME two editions' "Perfil de la VÍCTIMA/RESPONSABLE: sexo y nacionalidad" tables — a different, reliably-parseable table already handled by T26's `parse_country_breakdown_table` — parse cleanly with full country-level breakdown. `mir_parser.py`'s `InformeParser` was fixed so this data isn't silently dropped just because the unrelated typology table fails (see SPEC.md B41, `src/crime/SPEC-crime.md` T87); `sexual_crimes_mir_2017-2024.json` now carries real victim/perpetrator country breakdowns for 2017 and 2018, with `total_count=None` for those two years (this table stays the Anuario's job, per the table above — 11,692 for 2017, 13,782 for 2018). Sample: 2017 perpetrators — Marruecos 309 (Argelia not yet individually named, folded into Resto); 2018 perpetrators — Marruecos 564, Argelia 72 (first year Algeria breaks out individually in this series).
