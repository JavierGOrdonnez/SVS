...
| T11 | ~ | data | Operational-resource capacity (personnel/aircraft/vehicles) as a complementary metric to euro figures, per user direction after Madrid's budget-line opacity kept recurring. `data/raw/wff_operational_resources.csv` — 13 of 17 CCAAs, 2026 season, all press-sourced (`confidence=low`). See `data/sources/private_contractors_and_operations.md` for the full writeup, including a much stronger candidate source not yet pulled in: Civio's `todos-los-incendios-forestales` open dataset (EGIF-derived, per-fire personnel/equipment/extinction-cost, 1968-2023) — gated behind a registration form, not yet downloaded. |
| T12 | ~ | research | Private-contractor landscape (aerial-firefighting operators, TRAGSA-style state-owned entrustments, ground-crew subcontracting) — see `data/sources/private_contractors_and_operations.md`. One primary-sourced finding: Madrid's TRAGSA entrustment document (€107M, 2022-2025) pinpoints the exact budget line (134A/subconcepto 22706) that the earlier "Madrid doesn't isolate wildfire spending" finding (T9) had missed — revises that conclusion from "doesn't exist" to "exists but is generically labeled." Everything else in that file (Cartel del Fuego, CNMC investigation, Castilla y León's ~20-company privatization) is `confidence=low`/unverified leads from a single research pass, explicitly flagged as needing re-verification before citing as fact. |
