...
| `src/sexual_crimes/build_dashboard_data.py` | `sexual_crimes_mir_2017-2024.json` (incl. T-sx-rel's `relationship` field), `sexual_crime_evolution.csv`, `ine_condenados_28716_sexual_crimes.csv`, `macroencuesta_relationship_2015-2024.csv` (T-sx-rel-survey) | `build()` consumed by `src/analysis/build_dashboard.py` → `docs/data/sexual_crimes.json` (adds `relationship` — victim/perpetrator relationship, raw + known-cases-renormalized, 2017-2024; `survey_comparison` — Macroencuesta victim-reported relationship vs. MIR's own police-recorded "unknown" share, partner-excluded on both sides for a fair comparison) | T3,T21,T22,T-sx-rel,T-sx-rel-survey | x |
