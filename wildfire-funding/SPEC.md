...
| T9 | ~ | data | Executed vs. budgeted (C9) — schema extended and populated. 5 real presupuestado/liquidado pairs now computable (see `reports/wff_first_pass_2025_2026.md`'s Execution rate table): Castilla y León 2025 (48.5%, mid-year snapshot pre-fire-season), Cataluña 2021 (106.6%, broader-than-wildfire scope), Extremadura 2024 (2 pairs: 40.8% aggregate, 8.9% on the narrowest project line), País Vasco/Bizkaia 2023 (0€ initial credit → 1.34M executed via in-year credit, the clearest example of C9's core concern). All rows `confidence=low`, most pending `source_ref` re-verification (recovered from a crashed-agent research cache, or fetched fresh this pass) — see SOURCES_INDEX.md. Madrid and Aragón: real search effort made (Cámara de Cuentas, Cortes written answers, the region's own INFOMA operational plan), no presupuestado/liquidado pair found — documented as a gap. |
| T10 | ~ | infra/data | Tier-1 official all-CCAA total-budget parser (`parsers/parse_hacienda_totals.py`) — confirmed working by hand (both portals return real .xlsx downloads via a scripted session, not just interactive forms) but the actual run crashed on an environment-wide proxy outage after covering most of 2013-2024 in-memory; the script only wrote its output once at the end, so nothing was persisted. Needs a resumability fix (write incrementally per year) before rerunning — not yet done, tracked as the next infra task. |
